...
func _physics_process(delta: float) -> void:
	...
	GDebugOverlay.draw_with_origin(name + "wheel_direction", to_global(wheel_center_origin), -global_basis.z * 0.5, Color.ORANGE)
...
func process_suspension(delta: float, collision_point: Vector3, collision_normal: Vector3) -> void:
	if is_colliding():
			...
			var suspension_force = max(spring_force - damper_force, 0)
